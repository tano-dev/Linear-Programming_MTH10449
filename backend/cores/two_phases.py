...
class TwoPhases:
    """
    Solver using two phases algorithm
    """
    def __init__(self, problem: CanonicalProblem, bland: bool = False, verbose: bool=True):
        ...
        if problem.need_two_phases:
            ...
            self.M[:self.num_constraints, -1] = 1.0 # Cập nhật đúng cú pháp cho x_0
    ...
    def solve(self):
        ...
        if self.problem.need_two_phases:
            if self.verbose:
            ...
            if "x_0" in self.basic_vars:
                ...
                x0_idx_in_cols = self.non_basic_vars.index("x_0") + 1
                for j in range(1, self.M.shape[1]):
                    if j != x0_idx_in_cols and abs(self.M[r_x0, j]) > 1e-9:
                        self.pivot(r_x0, j)
                        if self.verbose:
                            self.print_dictionary("Phase 1")
                        break
            ...
            if self.verbose:
                print("---Pure Simplex---")
                self.print_dictionary("Pure")
            self.status = self._run_simplex(obj_row=self.num_constraints, phase_name="Pure")
